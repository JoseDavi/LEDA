...
public class BTreeImpl<T extends Comparable<T>> implements BTree<T> {
	...
	private BNode<T>[] listToArray(LinkedList<BNode<T>> list) {
		BNode[] nodesArray = new BNode[list.size()];
		for (int i = 0; i < list.size(); i++) {
			nodesArray[i] = list.get(i);
		}
		return nodesArray;
	}

	private void depthLeftOrder(BNode<T> node, LinkedList<BNode<T>> list) {
		if (!node.isEmpty()) {
			list.add(node);
