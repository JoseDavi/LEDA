...
public class HeapImpl<T extends Comparable<T>> implements Heap<T> {
	...
	private void heapify(int position) {
		...
		int maximo = position;
		if (left < size() && comparator.compare(heap[left], heap[maximo]) > 0) {
			maximo = left;
		}
		if (right < size() && comparator.compare(heap[right], heap[maximo]) > 0) {
			maximo = right;
		}
		if (maximo != position) {
	...
	public void insert(T element) {
		...
		if(element != null) {
			...
			heap[index] = element;
			int indice = index;
			int pai = parent(index);
			while(pai >= 0 && comparator.compare(heap[indice], heap[pai]) > 0) {
				Util.swap(heap, indice, pai);
				indice = pai;
				pai = parent(pai);
			}
		}
